...
func TestRender(t *testing.T) {
	...
	t.Run("WithSkinparam", func(t *testing.T) {
		...
		var buf bytes.Buffer
		err := goduml.Render(input, &buf,
			goduml.WithSkinparam("backgroundColor", "#FF0000"),
		)
		require.NoError(t, err)
	...
	t.Run("WithCustomTheme", func(t *testing.T) {
		...
		input := strings.NewReader("@startuml\nclass Foo\n@enduml")
		var buf bytes.Buffer
		err := goduml.Render(input, &buf, goduml.WithTheme(custom))
		require.NoError(t, err)
	...
	t.Run("ReadError", func(t *testing.T) {
		t.Parallel()
		var buf bytes.Buffer
		err := goduml.Render(&errReader{}, &buf)
		require.Error(t, err)
...
func TestParse(t *testing.T) {
	...
	t.Run("ValidDiagram", func(t *testing.T) {
		t.Parallel()
		input := strings.NewReader("@startuml\nclass Foo\n@enduml")
		diagram, errs := goduml.Parse(input)
	...
	t.Run("RenderAfterParse", func(t *testing.T) {
		...
		require.Empty(t, errs)
		var buf bytes.Buffer
		err := goduml.RenderDiagram(&buf, diagram)
		require.NoError(t, err)
