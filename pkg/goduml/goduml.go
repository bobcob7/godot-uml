// Package goduml provides the public library API for rendering PlantUML diagrams to SVG.
//
// The primary entry point is Render, which reads PlantUML input and writes SVG output:
//
//	err := goduml.Render(os.Stdin, os.Stdout)
//
// Use options to customize rendering:
//
//	err := goduml.Render(input, output,
//	    goduml.WithSkinparam("backgroundColor", "#FFFFFF"),
//	)
//
