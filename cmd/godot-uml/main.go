...
func printUsage() {
	fmt.Fprintln(os.Stderr, `Usage: godot-uml <command> [options]
...
Run 'godot-uml <command> --help' for command-specific help.`)
}

func cmdRender(args []string) int {
	outputFile, inputPath := parseRenderArgs(args)
	if inputPath == "" {
		fmt.Fprintln(os.Stderr, "Usage: godot-uml render <file.puml|-> [-o output.svg]")
		return exitSystem
	}
	var input *os.File
	if inputPath == "-" {
	...
	if err := goduml.Render(input, out); err != nil {
		fmt.Fprintf(os.Stderr, "error: %s\n", err)
...
func cmdValidate(args []string) int {
	...
	remaining := fs.Args()
	if len(remaining) == 0 {
		fmt.Fprintln(os.Stderr, "Usage: godot-uml validate <file.puml>")
		return exitSystem
	}
...
func cmdServe(args []string) int {
	...
	cfg.Host = *host
	srv := server.New(cfg)
	fmt.Fprintf(os.Stderr, "godot-uml server listening on http://%s:%d\n", cfg.Host, cfg.Port)
